...
function rebuild_tbody_desktop_vq4(id_tab,contenu){  //vq4 : pas de noms de mages 
	let table        = document.getElementById(id_tab);
	let tbody        = document.createElement("tbody");
	//for(const [idx,ligne] of contenu.lignes.entries() ){ //pour faciliter le travail de _vproxy
	for(let idx_l=0; idx_l<contenu.lignes.length ; idx_l++){
		let ligne = contenu.lignes[idx_l];
		let tr       = document.createElement("tr");
		//vq4 -> not : let th       = document.createElement ( "th" );
		//vq4 -> not : th.appendChild( document.createTextNode( contenu.noms[idx] ));
		//vq4 -> not : tr.appendChild( th );
		//for (const note of ligne ) {  //pour faciliter le travail de _vproxy
		for(let idx_note=0; idx_note<ligne.length ; idx_note++ ){
			let note = ligne[idx_note];
			let td   = document.createElement("td");
			let c_n  = document.createTextNode(note);
			td.appendChild( c_n );
			tr.appendChild( td  );
...
function get_table_in_desktop_view(id_table){
	fetch(
		...
		json => { 
			console.log(json);
			//let contenu = adaptateur_contenu(json);
			let contenu = adaptateur_contenu_vproxy(json);
			trash_and_rebuild_thead_desktop_vq4(id_table,contenu);
			rebuild_tbody_desktop_vq4(id_table,contenu);
		}
	).catch(
		err => { console.log(err); }
	);
...
function get_table_in_mobile_view(id_table){
	fetch(
		...
		json => { 
			...
			//let contenu = adaptateur_contenu(json);
			let contenu = adaptateur_contenu_vproxy(json);
			trash_and_rebuild_thead_mobile_vq4(id_table,contenu);
			rebuild_tbody_mobile_vq4(id_table,contenu);
		}
	).catch(
		err => { console.log(err); }
	);
...
function adaptateur_contenu(json){
	let contenu = { colonnes : json.disciplines, lignes : [] };
	for(const mage of json.premiere_annee){
		contenu.lignes.push( mage.notes );
	}
	return contenu;
}

function adaptateur_contenu_vproxy(json){
	console.log('cree adaptateur via proxy');
	return {
		get colonnes(){
			return new Proxy(json, {
				get:function(target,idx_disc,reciever){
					if(idx_disc==='length'){
						return target.disciplines.length
					}
					return target.disciplines[idx_disc]
				}
			})
		},
		get lignes(){
			return new Proxy(json, {
				get:function(target,idx_ligne,reciever){
					if(idx_ligne==='length'){
						return target.premiere_annee.length;
					}
					return 	new Proxy(json, {
						get:function(target,idx_name,reciever){
							if(idx_name==='length'){
								return target.premiere_annee[idx_ligne].notes.length
							}
							return target.premiere_annee[idx_ligne].notes[idx_name]
						}
					})
				}
			})
